...
**Phase 3 is DONE and LOCKED.** `src/scorer.py` grades a reasoning method's `{malicious_events, narrative}` output against `data/ground_truth/{scenario}.json`, producing event-level P/R/F1, stage-level P/R/F1 + partial-credit recall, and hallucination rate (never blended into one composite score). `tests/test_scorer.py` hand-verifies both a known-good and a known-bad case against WS12's real ground truth — the known-bad test caught a real bug in the hallucination-IOC regex on its first run (it was also matching numeric fragments of an IP address as fake "filenames"). **Do not modify `scorer.py`'s scoring logic to change what it reports for any specific reasoning method's output** — any change must be justified independently. Full detail: `IMPLEMENTATION.md`.
